...
class Model():
  ...
  def keyframe_detection(self, video_path):
    ...
    if self.deployment_detector:
      ...
      if fps > 0:
        print(f"  ⏭️  Skipping: 0s-{start_frame/fps:.1f}s (deployment) and {end_frame/fps:.1f}s-end (retrieval)")
    ...
    if skipped_frames > 0:
      print(f"\n  ✂️  Skipped {skipped_frames} frames during deployment/retrieval")
  ...
  def track_video(self, video_path):
    ...
    sightings = []
    raw_detections = []  # Log ALL detections before tracking/filtering
    skipped_frames = 0
    ...
    for frame, time, frame_idx in tqdm(video_iterator, total=tot_frames_to_process):
      ...
      # Extract raw detections (before tracking filters) for QC logging
      raw_detections += extract_raw_detections(original_video_path, self.input_path, frame_results, frame_idx, ms_to_string(time))

      # Extract tracked sightings (with track IDs)
      sightings += extract_sightings(original_video_path, self.input_path, frame_results, frame_idx, ms_to_string(time), **{"tracking": True})

    if skipped_frames > 0:
      print(f"  ✂️  Skipped {skipped_frames} frames during deployment/retrieval")

    # Save raw detections log for QC
    if raw_detections:
    ...
    cleanup_converted_video(video_path, original_video_path, verbose=True)
